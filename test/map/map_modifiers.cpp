...
void test_map_modifiers()
{
    ...
    m.insert(ft::make_pair(25, "Twenty-Five"));
    ...
    if (!m.empty())
    {
        ft::map<int, std::string>::iterator first = m.begin();
        ft::map<int, std::string>::iterator last = m.begin();
        std::advance(last, 2);
        m.erase(first, last);
    ...
    ft::map<int, std::string>::iterator f = m.find(25);
    if (f != m.end())
    ...
    other.insert(ft::make_pair(1, "One"));
    other.insert(ft::make_pair(2, "Two"));
